			    ...
			    struct arm_smccc_res *res)
{
	...
	index = smc_avail_ring->ring[smc_avail_ring->head];
	...
	smc_avail_ring->ring[smc_avail_ring->head] = OPTEE_SHM_QUEUE_SIZE;

	smc_avail_ring->head = (smc_avail_ring->head + 1) % OPTEE_SHM_QUEUE_SIZE;
	...
	spin_lock(&smc_ring_lock);
	smc_used_ring->ring[smc_used_ring->tail] = index;
	smc_used_ring->tail = (smc_used_ring->tail + 1) % OPTEE_SHM_QUEUE_SIZE;
	spin_unlock(&smc_ring_lock);

	writel(smc_vm_ids->tee_id << 16, g_ivshmem_dev.regs_addr + DOORBELL_OFF);
	...
	spin_lock(&smc_ring_lock);
	smc_avail_ring->ring[smc_avail_ring->tail] = index;
	//pr_info("move index %d back to avail tail %d\n", index, smc_avail_ring->tail);
	smc_avail_ring->tail = (smc_avail_ring->tail + 1) % OPTEE_SHM_QUEUE_SIZE;
	spin_unlock(&smc_ring_lock);
}
#else
static void optee_smccc_smc(unsigned long a0, unsigned long a1,
...
static int ivshmem_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	smc_vm_ids = (struct optee_vm_ids *)g_ivshmem_dev.base_addr;
	smc_avail_ring = (struct optee_smc_ring *)(g_ivshmem_dev.base_addr +
		sizeof(struct optee_vm_ids));
	smc_used_ring = (struct optee_smc_ring *)(g_ivshmem_dev.base_addr +
	...
	for (i = 0; i < OPTEE_SHM_QUEUE_SIZE; i++) {
		smc_avail_ring->ring[i] = i;
	}
	smc_used_ring->head = 0;
	smc_used_ring->tail = 0;
	for (i = 0; i < OPTEE_SHM_QUEUE_SIZE; i++) {
		smc_used_ring->ring[i] = OPTEE_SHM_QUEUE_SIZE;
	}
	...
	if (g_ivshmem_dev.revision == 1) {
		...
		smc_vm_ids->ree_id = g_ivshmem_dev.ivposition;
		pr_info("ivshmem device ree id=%d, tee id=%d\n",
				smc_vm_ids->ree_id, smc_vm_ids->tee_id);
