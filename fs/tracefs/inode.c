...
static int tracefs_permission(struct user_namespace *mnt_userns,
...
static int tracefs_getattr(struct user_namespace *mnt_userns,
...
static int tracefs_setattr(struct user_namespace *mnt_userns,
...
static int tracefs_apply_options(struct super_block *sb, bool remount)
{
	...
	if (remount && (opts->opts & BIT(Opt_uid) || opts->opts & BIT(Opt_gid))) {
		...
		list_for_each_entry_rcu(ti, &tracefs_inodes, list) {
			if (update_uid) {
				ti->flags &= ~TRACEFS_UID_PERM_SET;
				ti->vfs_inode.i_uid = opts->uid;
			}

			if (update_gid) {
				ti->flags &= ~TRACEFS_GID_PERM_SET;
				ti->vfs_inode.i_gid = opts->gid;
			}
